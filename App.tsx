...
const getRatingColor = (rating: InvestmentRating): string => {
  switch (rating) {
...
// FIX: StockCard now receives onSelect instead of onClick so the stable callback
// reference from the parent prevents React.memo from seeing a new prop each render.
const StockCard = React.memo<{ stock: Stock; onSelect: (stock: Stock) => void }>(({ stock, onSelect }) => {
  // Stable per-instance handler: only recreated if onSelect or stock changes.
  const handleClick = useCallback(() => onSelect(stock), [onSelect, stock]);
...
// --- HomeView ---
// Extracted as a proper memoized component so React can skip re-rendering it
// entirely when the ANALYSIS view is active, and vice versa.
const HomeView = React.memo<{ stocks: Stock[]; onSelectStock: (stock: Stock) => void }>(({ stocks, onSelectStock }) => (
...
interface AnalysisViewProps {
  ...
  onShowBuyPopup: (show: boolean) => void;
}

const AnalysisView = React.memo<AnalysisViewProps>(({ stock, slide, showBuyPopup, onShowBuyPopup }) => {
  // All derived values are cheap to compute and depend only on the stable `stock` prop.
  const histData = stock.hist || [];
  const scenariosData = stock.scenarios || [];
  const rsRating = stock.rs || 0;
  ...
  const is3Y = stock.ticker === 'ANET';

  // alertTags lives here (not in App) since it only depends on this component's prop.
  const alertTags = useMemo(() =>
    stock.dnaTags?.filter(t => t.includes('🔴') || t.includes('GM') || t.includes('Risk') || t.includes('Conc.')) || []
  , [stock.dnaTags]);

  // Stable callbacks for the modal so child buttons never cause unnecessary work.
  const handleOpenBuy = useCallback(() => onShowBuyPopup(true), [onShowBuyPopup]);
  const handleCloseBuy = useCallback(() => onShowBuyPopup(false), [onShowBuyPopup]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-500 pb-24">
      ...
      {slide === 0 && (
        <div className="space-y-10">
          <div className="text-center">
            <span className="text-blue-500 text-[10px] font-black uppercase tracking-[0.3em] mb-4 block">{stock.ticker} Strategic Snapshot</span>
            <h2 className="text-6xl md:text-7xl font-black text-white mt-2 leading-none tracking-tighter">
              {stock.name}
            </h2>
            <p className="text-slate-500 text-xl mt-4 font-medium">{stock.typeLabel}</p>
            {stock.dnaTags && (
              <div className="flex flex-wrap justify-center gap-2 mt-10">
                {stock.dnaTags.map((t, i) => <Tag key={i} label={t} />)}
              </div>
            )}
          </div>

          <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-5">
            {stock.stats.filter(s => s.label !== "RS RATING").map((st, idx) => (
              <Chip key={idx} label={st.label} val={st.value} color={st.label.includes('TARGET') || st.label.includes('VALUE') ? '#10b981' : '#3b82f6'} />
            ))}
            <Chip label="BETA" val={stock.beta.toString()} color="#a855f7" />
            <Chip label="UPSIDE (1Y)" val={stock.momentumUpside1Y} color="#38bdf8" />
            <Chip label="EST. VELOCITY" val={stock.timeToMilestone} color="#f59e0b" />
      ...
      {slide === 4 && (
        <div className="space-y-10">
          <div className="bg-gradient-to-br from-[#1e1b4b] to-[#080d1a] p-16 md:p-24 rounded-[3rem] text-center border border-blue-500/20 shadow-2xl relative overflow-hidden">
            ...
            <div className="text-slate-300 max-w-4xl mx-auto mb-16 text-xl md:text-2xl leading-relaxed font-medium italic text-center">"{stock.verdict}"</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
              ...
              <div className="bg-blue-600/10 backdrop-blur-sm p-10 rounded-[2.5rem] border border-blue-500/30 shadow-xl scale-110 relative z-10 flex flex-col justify-center items-center">
                ...
                <div className="text-5xl font-black text-blue-400 tracking-tighter">{stock.momentumUpside1Y}</div>
                <div className="mt-4 pt-4 border-t border-blue-500/20 w-full text-center">
                  ...
                  <div className="text-sm font-bold text-slate-300">{stock.timeToMilestone}</div>
                </div>
              </div>
              <div className="bg-slate-900/60 backdrop-blur-sm p-10 rounded-[2.5rem] border border-slate-800 shadow-xl">
                ...
                <div className="text-5xl font-black text-emerald-400 tracking-tighter">{formatAccelerationScore(stock.accelerationProb)}</div>
              </div>
            </div>
      ...
      {showBuyPopup && (
        ...
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300"
        ...
        >
          ...
          >
            ...
            <div className="flex flex-col items-center mb-8 relative z-10 text-center">
              ...
              <h2 className="text-white text-4xl md:text-5xl font-black tracking-tighter">{stock.ticker} Thesis</h2>
              <button
                onClick={handleCloseBuy}
...
export default function App() {
  ...
  const [stocks] = useState<Stock[]>(() => {
    // FIX: wrap in try-catch so corrupted localStorage data doesn't crash the app.
    try {
  ...
  // FIX: track both timer IDs so the cleanup clears whichever fired last.
  useEffect(() => {
    let fadeTimer: ReturnType<typeof setTimeout>;
    const timer = setTimeout(() => {
      setFadeOut(true);
      fadeTimer = setTimeout(() => setIsInitialLoading(false), 300);
    }, 1000);
    return () => {
      clearTimeout(timer);
  ...
  // FIX: stable callback reference prevents StockCard's React.memo from
  // seeing a new onClick prop on every parent render.
  ...
  // Stable nav handler so nav buttons don't recreate on every render.
  const handleSlideNav = useCallback((i: number) => {
    setSlide(i);
    setShowBuyPopup(false);
  }, []);
    ...
    <>
      ...
      <div className="flex flex-col h-screen overflow-hidden bg-[#080d1a] text-slate-200 select-none font-inter selection:bg-blue-500 selection:text-white">
        ...
        <main className="flex-1 overflow-y-auto">
          {view === 'HOME' && <HomeView stocks={stocks} onSelectStock={handleSelectStock} />}
          {view === 'ANALYSIS' && selectedStock && (
            <div className="px-6 py-12 md:px-10">
              <AnalysisView
                ...
                onShowBuyPopup={setShowBuyPopup}
              />
            </div>
          )}
        </main>
