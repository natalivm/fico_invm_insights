...
export default function App() {
  ...
  const renderAnalysis = () => {
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-500 pb-24">
        ...
        {slide === 4 && (
          <div className="bg-gradient-to-br from-[#1e1b4b] to-[#080d1a] p-16 rounded-[3rem] text-center border border-blue-500/20 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-blue-500 to-transparent"></div>
            
            <h3 className="text-5xl font-black mb-8 text-white uppercase tracking-tighter">Investment Verdict</h3>
            <div className="text-slate-300 max-w-3xl mx-auto mb-16 text-xl leading-relaxed font-medium italic">{selectedStock.verdict}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
              ...
              <div className="bg-blue-600/10 backdrop-blur-sm p-10 rounded-[2.5rem] border border-blue-500/30 shadow-xl scale-105">
            ...
            <button 
              onClick={backToHome} 
              className="group bg-white text-slate-950 px-12 py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs hover:bg-blue-500 hover:text-white transition-all duration-300 shadow-2xl flex items-center gap-4 mx-auto"
            >
              Back to Portfolio
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-[#080d1a] text-slate-200 select-none font-inter selection:bg-blue-500 selection:text-white">
      ...
      {view === 'ANALYSIS' && (
        <nav className="flex justify-center gap-3 p-4 bg-[#0e1829]/50 backdrop-blur-md border-b border-[#1e3251] overflow-x-auto no-scrollbar shrink-0 z-10">
          {SLIDES.map((s, i) => (
            <button 
              key={i} 
              onClick={() => setSlide(i)} 
              className={`px-8 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all whitespace-nowrap tracking-widest ${slide === i ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30 ring-2 ring-blue-500/20' : 'bg-slate-900/50 text-slate-500 hover:text-slate-300 hover:bg-slate-800'}`}
            >
              {s}
            </button>
          ))}
